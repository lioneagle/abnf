...
func (this *KeyCmpGeneratorForGolang) generateFile(config *key_gen.Config,
	...
	if config.GenVersion {
		this.Fprintfln(file, "/*%s*/", gen.GenerateVersion())
	}
...
func (this *KeyCmpGeneratorForGolang) GenerateIndex(config *key_gen.Config,
	...
	if len(config.MaxIndexName) > 0 {
		if len(config.MaxIndexName) > MaxIndexName {
			MaxIndexName = len(config.MaxIndexName)
		}
	}
	...
	if len(config.MaxIndexName) > 0 {
		this.Fprintf(w, "%s", config.MaxIndexName)
		basic.PrintIndent(w, MaxIndexName-len(config.MaxIndexName))
		fmt.Fprintf(w, " %s = iota", indexTypeName)
		this.PrintReturn(w)
	}
...
func (this *KeyCmpGeneratorForGolang) GenerateActionDeclaration(config *key_gen.Config, w io.Writer) {
	...
	cursorType := getCursorTypeName(config)

	if config.CharsetEnabled {
		this.Fprintf(w, "func %s(%s %s, %s %s) (%s, %s)", config.ActionName,
			config.SrcName, srcType,
			config.CursorName, cursorType,
...
func (this *KeyCmpGeneratorForGolang) GenerateActionDefinition(config *key_gen.Config,
	...
	if !config.CharsetEnabled {
		if strings.Compare(cursorType, "int") == 0 {
			this.Fprintfln(w, "%s := 0", config.CursorName)
		} else {
			this.Fprintfln(w, "%s := %s(0)", config.CursorName, cursorType)
		}
	}
	...
	if strings.Compare(cursorType, "int") == 0 {
		...
		this.Fprintfln(w, "len1 := %s(len(%s))", cursorType, srcName)
	}
	...
	this.generateLeftBrace(config, w, config.IndentOfBlock)
	if config.CharsetEnabled {
		this.Fprintfln(w, "return %s, %s", config.UnknownIndexName, config.CursorName)
	} else {
...
func (this *KeyCmpGeneratorForGolang) GenerateActionCode(config *key_gen.Config,
	...
	if branch != nil {
		if config.SeperatorEnabled {
			this.Fprintf(w, "if (%s < len1) && %s(%s[%s])", cursorName, config.SeperatorCharsetName, srcName, cursorName)
		} else if config.CharsetEnabled {
			this.Fprintf(w, "if (%s >= len1) || !%s(%s[%s])", cursorName, config.CharsetName, srcName, cursorName)
		} else {
		...
		if config.CharsetEnabled {
			this.Fprintfln(w, "return %s, %s", branch.Next.Key.Index.Name, cursorName)
		} else {
		...
		if depth == 0 && len(tree.Branches) == 1 {
			...
			//this.Fprintfln(w, "*%s = %s;", srcName, cursorName)
			if config.CharsetEnabled {
				this.Fprintfln(w, "return %s, %s", config.UnknownIndexName, cursorName)
			} else {
				this.Fprintfln(w, "return %s", config.UnknownIndexName)
			}
			this.GenerateBlockEnd(config, w)
...
func getCursorTypeName(config *key_gen.Config) string {
	if len(config.CursorTypeName) > 0 {
		return config.CursorTypeName
	}

	return getTypeNameBySize(config.CursorTypeSize)
}
